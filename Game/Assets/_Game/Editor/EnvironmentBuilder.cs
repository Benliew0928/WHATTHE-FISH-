...
public static partial class ProjectBuilder {
 static void ConfigureEnvironments(AppRoot app,GameObject football,GameObject basketball,GameObject golf,Light light,Camera camera){
  ...
  var sports=new[]{SportId.Football,SportId.Basketball,SportId.Golf};env.roots=new[]{football,basketball,golf};env.definitions=new SportDefinition[sports.Length];
  for(int i=0;i<sports.Length;i++){
   ...
   def.farClip=450;def.fogStart=180;def.fogEnd=430;def.elevatedDistance=19;def.indoor=false;def.lightIntensity=1.1f;def.lightRotation=new Vector3(48,-35,0);
