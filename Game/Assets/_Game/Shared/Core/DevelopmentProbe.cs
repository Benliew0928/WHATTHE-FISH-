...
namespace SportsPrototype {
 ...
 public sealed class DevelopmentProbe:MonoBehaviour {
  ...
  void Start(){args=Environment.GetCommandLineArgs();output=Value("-report",Application.persistentDataPath+"/probe.jsonl");int.TryParse(Value("-expected","1"),out expected);if(args.Contains("-smoke"))StartCoroutine(Smoke());if(args.Contains("-cloudHost")||args.Contains("-cloudCode"))StartCoroutine(Cloud());}
  IEnumerator Cloud(){yield return null;var room=AppRoot.Instance.rooms;var task=args.Contains("-cloudHost")?room.Create():room.Join(Value("-cloudCode",""));while(!task.IsCompleted)yield return null;Record("CLOUD_ROOM code="+room.Code+" connected="+room.Connected+" error="+room.Error);if(room.Connected&&args.Contains("-codeFile"))File.WriteAllText(Value("-codeFile","room-code.txt"),room.Code);}
