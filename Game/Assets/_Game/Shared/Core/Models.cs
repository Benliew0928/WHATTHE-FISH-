...
namespace SportsPrototype {
 ...
 [Serializable] public struct StadiumAppearance { public string title; public int palette,design,screen; public bool flags; public static StadiumAppearance Default=>new StadiumAppearance{title="SUNNY PARK",flags=true}; public void Clamp(){ title=string.IsNullOrWhiteSpace(title)?"SUNNY PARK":title.Trim();if(title.Length>24)title=title.Substring(0,24);palette=Mathf.Clamp(palette,0,3);design=Mathf.Clamp(design,0,2);screen=Mathf.Clamp(screen,0,1); } }
 public struct PlayerCommand { public Vector2 move; public float heading; public bool sprint; }
