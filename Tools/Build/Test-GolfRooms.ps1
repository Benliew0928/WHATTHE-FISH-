...
$out=Join-Path $root "Builds\GolfQA-$Transport-$runId"
New-Item -ItemType Directory -Path $out | Out-Null
$processes=[Collections.Generic.List[System.Diagnostics.Process]]::new()
$byName=@{}
$results=[Collections.Generic.List[string]]::new()
function Launch([string]$name,[string]$role,[int]$expected,[int]$seconds,[string]$sport='Basketball',[int]$start=80,[int]$return=105){
 $arguments="-batchmode -nographics -job-worker-count 2 -probe -sport $sport $role -authProfile golf$runId$name -expected $expected -startAt $start -returnAt $return -report $out\$name.txt -exitAfter $seconds -logFile $out\$name.log"
...
try {
 if($Transport -eq 'Cloud'){
  Launch 'Ahost' "-cloudHost -codeFile $out\Acode.txt -leaveAfter 165" 10 170 'Golf' 90 112
  Await 'Ahost' 'CLOUD_ROOM code=[A-Z0-9]+ connected=True'
  $codeA=(Get-Content "$out\Acode.txt" -Raw).Trim();$joinA="-cloudCode $codeA"
 }else{
  Launch 'Ahost' '-localHost -port 7897 -leaveAfter 165' 10 170 'Golf' 90 112
  Await 'Ahost' 'count=1 connected=True';$joinA='-localClient -port 7897'
 }
 # Guests deliberately start on Basketball; room authority must select Golf.
 ...
 foreach($i in 1..9){Await "Aguest$i" 'sport=Golf activeEnvironments=1 world=.*"title":"ISLAND GREENS"' 25}
 Pass 'Ten-player golf room; host sport and island name received by all nine guests'
 ...
 # A second, simultaneous basketball room must keep its own state.
 if($Transport -eq 'Cloud'){
  Launch 'Bhost' "-cloudHost -codeFile $out\Bcode.txt -leaveAfter 95" 2 100 'Basketball' 20 60
  Await 'Bhost' 'CLOUD_ROOM code=[A-Z0-9]+ connected=True'
  $codeB=(Get-Content "$out\Bcode.txt" -Raw).Trim();$joinB="-cloudCode $codeB"
 }else{
  Launch 'Bhost' '-localHost -port 7898 -leaveAfter 95' 2 100 'Basketball' 20 60
  Await 'Bhost' 'count=1 connected=True';$joinB='-localClient -port 7898'
 }
 Launch 'Bguest' $joinB 2 155 'Golf'
 Await 'Bguest' 'count=2 connected=True exploring=True.*sport=Basketball'
 Pass 'Separate basketball room adopts Basketball and starts independently'
 Launch 'Locked' $joinB 2 18
 Await 'Locked' 'locked|Exploration has started' 25;Pass 'Joining during exploration rejected'
 Await 'Aguest1' 'count=10 connected=True exploring=True.*sport=Golf' 110
 Pass 'Golf host starts all players'
 Await 'Ahost' 'time=11[4-9].*exploring=False' 60
 Await 'Aguest1' 'time=11[4-9].*exploring=False' 35
 Pass 'Golf host returns guests to waiting room'
 ...
 # Free one place, then check a new guest receives golf and its fixed identity after return.
 Await 'Ahost' 'time=1[234][0-9].*count=9 connected=True exploring=False' 35
 Launch 'LateGuest' $joinA 10 65 'Basketball'
 Await 'LateGuest' 'count=10 connected=True exploring=False.*sport=Golf.*"title":"ISLAND GREENS"' 25
 Pass 'Joining after return receives golf and the island name'
