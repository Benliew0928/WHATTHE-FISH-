...
$processes=@()
function Start-TackleProbe([string]$Name,[string]$Options){
...
try {
    ...
    if($processes | Where-Object {-not $_.HasExited}){throw 'Tackle probes timed out'}
    foreach($name in @('gameplay','host','client')){
        ...
        $required=if($name -eq 'gameplay'){'TACKLE_GAMEPLAY_COMPLETE'}else{'NETWORK_TACKLE_COMPLETE'}
    ...
    Write-Output "PASS tackle gameplay and two-player checks: $out"
} finally {
